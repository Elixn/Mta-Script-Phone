...
boneAttach = exports.bone_attach
...
local smallestDBID = function(dataTable)
    if dataTable then
        local query = dbQuery(connection:getConn(), "SELECT MIN(e1.id+1) AS nextID FROM "..dataTable.." AS e1 LEFT JOIN "..dataTable.." AS e2 ON e1.id +1 = e2.id WHERE e2.id IS NULL")
...
local updatePhoneDatas = function(player, phone)
    ...
    {player}, connection:getConn(), "SELECT * FROM phone_contacts WHERE phone = ?", phone)
    ...
    {player}, connection:getConn(), "SELECT * FROM phone_historys WHERE phone = ?", phone)
    ...
    {player}, connection:getConn(), "SELECT * FROM phone_sms")
end
...
local syncAdverts = function()
    triggerClientEvent(root, "phone.sync.adverts", root, adverts)
end
...
local updateSmsDetails = function(player, phone, num)
    ...
    {source}, connection:getConn(), "SELECT * FROM phone_sms_details")
end
...
addEvent('phone.advert.send', true)
addEventHandler('phone.advert.send', root, function(message)
    table.insert(adverts, {source.name, message})
    syncAdverts()
end)
...
addEventHandler('phone.call', root, function(num)
    if tonumber(num) then
        ...
        dbExec(connection:getConn(), "INSERT INTO phone_historys SET id='"..(smallestDBID("phone_historys")).."', phone='"..(source:getData('lastPhoneId')).."', number='"..(num).."', hour='"..hours.."', minute='"..minutes.."'")
        ...
        exports.pass:completeTask(source, 9)
        if tonumber(num) == 911 then
            source:outputChat('#D0D0D0Telefon: Los Santos Police Department lütfen yaşadığınız sorunu bildirin.',195,184,116,true)
            source:setData('callWaiting', nil)
            source:setData('call.services', true)
        elseif tonumber(num) == 411 then
            source:outputChat('#D0D0D0Telefon: Los Santos Medical Department lütfen yaşadığınız sorunu bildirin.',195,184,116,true)
            source:setData('callWaiting', nil)
            source:setData('call.services', true)
        elseif tonumber(num) == 912 then
            source:outputChat('#D0D0D0Telefon: Los Santos Sheriffs Department lütfen yaşadığınız sorunu bildirin.',195,184,116,true)
            source:setData('callWaiting', nil)
            source:setData('call.services', true)
        else
            source:setData('call.services', nil)
            for _, player in ipairs(Element.getAllByType('player')) do
                if global:hasItem(player, 2, source:getData('call.num')) then
                    if player == source then
                        ...
                        dbExec(connection:getConn(), "INSERT INTO phone_historys SET id='"..(smallestDBID("phone_historys")).."', phone='"..(source:getData('call.num')).."', number='"..(source:getData('lastPhoneId')).."', hour='"..hours.."', minute='"..minutes.."'")
...
addEventHandler('remove.phonedata', root, function()
    local datas = {'lastPhoneId', 'phone.viewed.num', 'phone.target.number', 'called', 'call.num', 'call.services', 'callTarget', 'callWaiting', 'caller'}
...
addEventHandler('phone.add.contact', root, function(name, num)
    if name and tonumber(num) then
        dbExec(connection:getConn(), "INSERT INTO phone_contacts SET id='"..(smallestDBID("phone_contacts")).."', phone='"..(source:getData('lastPhoneId')).."', name='"..(name).."', number='"..(num).."'")
        updatePhoneDatas(source, source:getData('lastPhoneId'))
    else
        source:outputChat('►#D0D0D0 Bir şeyler ters gitti!',195,184,116,true)
    end
...
addEventHandler('phone.delete.contact', root, function(phone, db)
    if tonumber(db) then
        dbExec(connection:getConn(), "DELETE FROM phone_contacts WHERE id='" ..(db).. "' LIMIT 1")
        updatePhoneDatas(source, phone)
    else
        source:outputChat('►#D0D0D0 Bir şeyler ters gitti!',195,184,116,true)
    end
...
addEventHandler('phone.delete.history', root, function(phone, db)
    if tonumber(db) then
        dbExec(connection:getConn(), "DELETE FROM phone_historys WHERE id='" ..(db).. "' LIMIT 1")
        updatePhoneDatas(source, phone)
    else
        source:outputChat('►#D0D0D0 Bir şeyler ters gitti!',195,184,116,true)
    end
...
addEventHandler('phone.sms.viewed', root, function(phone, db)
    if tonumber(db) then
        dbExec(connection:getConn(), "UPDATE `phone_sms_details` SET `viewed`='2' WHERE id='" ..(db).. "' LIMIT 1")
        updatePhoneDatas(source, phone)
    else
        source:outputChat('►#D0D0D0 Bir şeyler ters gitti!',195,184,116,true)
    end
...
addEventHandler('phone.new.message', root, function(num, message)
    if tonumber(num) and message then
        ...
        dbExec(connection:getConn(), "INSERT INTO phone_sms SET id='"..(smallestDBID("phone_sms")).."', phone='"..(source:getData('lastPhoneId')).."', number='"..(source:getData('phone.target.number')).."'")
        dbExec(connection:getConn(), "INSERT INTO phone_sms_details SET id='"..(smallestDBID("phone_sms_details")).."', phone='"..(source:getData('lastPhoneId')).."', number='"..(source:getData('phone.target.number')).."', message='"..(message).."', hour='"..(hours).."', minute='"..(minutes).."'")
        updatePhoneDatas(source, source:getData('lastPhoneId'))
        for _, player in ipairs(Element.getAllByType('player')) do
        ...
        dbQuery(
			function(qh, source)
				...
				if rows > 0 then
                    ...
                    dbExec(connection:getConn(), "INSERT INTO phone_settings SET phone='"..(source:getData('lastPhoneId')).."', password='"..(password).."'")
                    source:outputChat('►#D0D0D0 Telefonunuzun yeni şifresi '..password..' olarak belirlendi.',195,184,116,true)
                    triggerClientEvent(source, 'phone.success.password', source)
                end
			end,
		{source}, connection:getConn(), "SELECT * FROM phone_settings WHERE phone = ?", source:getData('lastPhoneId'))
    else
        source:outputChat('►#D0D0D0 Bir şeyler ters gitti!',195,184,116,true)
    end
end)