...
function phone:constructor()
    ...
    local events = {'phone.success.password', 'phone.sync.twitter', 'phone.sync.adverts', 'phone.contact.data', 'phone.history.data', 'phone.sms.data', 'phone.smsdetails.data', 'phone.ring', 'phone.open'}
    ...
    addEventHandler('phone.sync.adverts', root, function(...) self:advertSync(...) end)
...
function phone:display()
    ...
    if localPlayer:getData('called') then
    ...
    elseif localPlayer:getData('caller') then
            if localPlayer:getData('callWaiting') then
    ...
    elseif self.page == 7 then
        self.selectedText = 'message'
        self.selectedTable = self.adverts
        self.counter = 0
        self.counterY = 0
        self.maxScroll = 4
        for index, value in ipairs(self.adverts) do
            if index > self.scroll and self.counter < self.maxScroll then
                self:roundedRectangle(self.sizeX+35, self.sizeY-50-435+self.counterY, 200, 65, 8, tocolor(123,63,226,250))
                --dxDrawImage(self.sizeX+210, self.sizeY-50-430+self.counterY, 15, 15, 'components/images/tweet.png', 0, 0, 0, tocolor(35,35,35,250))
                dxDrawText(''..value[1]..' ilanladı', self.sizeX+40, self.sizeY-50-430+self.counterY, 25, 25, tocolor(25,25,25,250), 0.75, self.roboto)
                dxDrawText(value[2], self.sizeX+40, self.sizeY-50-415+self.counterY, 25, 25, tocolor(25,25,25,250), 0.75, self.roboto)
                self.counter = self.counter + 1
                self.counterY = self.counterY + 70
            end
        end
        dxDrawText('İlana numaranızı yazmayı unutmayın;', self.sizeX+35, self.sizeY-50-150, 25, 25, tocolor(200,200,200,255), 0.75, self.robotoB)
        if self:isInBox(self.sizeX+35, self.sizeY-50-130, 200, 60) then
            self:roundedRectangle(self.sizeX+35, self.sizeY-50-130, 200, 60, 8, tocolor(17,17,17,250))
            if getKeyState('mouse1') and self.click+600 <= getTickCount() then
                self.click = getTickCount()
        ...
        if string.len(self.messageText) > 0 then
            if getKeyState('enter') and self.click+600 <= getTickCount() then
                self.click = getTickCount()
                triggerServerEvent('phone.advert.send', localPlayer, self.messageText)
                self.messageText = ''
                self.lineText = 27
            end
        end
